...
def train_diabetes_model():
    ...
    with open("saved_models/diabetes_features.pkl", "wb") as f:
...
def explain_diabetes(input_data):
    with open("saved_models/diabetes_model.pkl", "rb") as f:
        model = pickle.load(f)
    with open("saved_models/diabetes_scaler.pkl", "rb") as f:
        scaler = pickle.load(f)
    with open("saved_models/diabetes_features.pkl", "rb") as f:
        features = pickle.load(f)
    input_array = np.array(input_data, dtype=float).reshape(1, -1)
    input_scaled = scaler.transform(input_array)
    
    # Use feature importances instead of SHAP to avoid compatibility issues
